...
## 📣 Latest News
- **[Nov 17, 2026]**: 🔄 During an internal reproduction check (downloading the dataset from HuggingFace as a user would), we noticed that the [**VTBench**](https://huggingface.co/datasets/We-Math/VTBench) subset for the Instruction-Guided Interaction task on HF corresponds to a non-final export and does not match the finalized version used in our experiments. We are re-verifying the full subset and plan to release the corrected version tomorrow (Nov 18). 🙇 We apologize for the inconvenience and appreciate your understanding. 🚧
