...
class VQAORM(ORM):
    ...
    async def evaluate(self, cur_completion, cur_solution, cur_port, image, question) -> float:
        ...
        # with open("/mnt/cephfs1/runqi/nanata/data/tool_rl/thyme_qwen2_5/Thyme/thyme_output/vqaorm/vqaorm.log.jsonl", "w") as fout:
...
class FMTORM(ORM):
    ...
    async def evaluate(self, cur_completion) -> float:
        ...
        # with open("/mnt/cephfs1/runqi/nanata/data/tool_rl/thyme_qwen2_5/Thyme/thyme_output/fmtorm/Format_Rewardcode_all_math_8.jsonl", "w") as fout:
...
class CODEORM(ORM):
    ...
    async def evaluate(self, cur_completion) -> float:
        ...
        if self.norm:
            code_r = 2 * code_r - 1
        # with open("/mnt/cephfs1/runqi/nanata/data/tool_rl/thyme_qwen2_5/Thyme/thyme_output/codeorm/Code_Reward.jsonl", "w") as fout:
        #     fout.write(json.dumps({"cur_completion": cur_completion})+"\n")
