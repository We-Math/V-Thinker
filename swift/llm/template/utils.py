...
class StopWordsCriteria(StoppingCriteria):
    ...
    def __call__(self, input_ids: torch.Tensor, scores: torch.Tensor, **kwargs) -> torch.Tensor:
        ...
        for i, text in enumerate(text_list):
            ...
            is_finished = False
            print('-' * 20)
            print(f"[Step debug] text[{i}] = '{repr(text)}'")
            for stop_word in self.stop_words:
                print(f"[DEBUG] stop_word type={type(stop_word)}, stop_word={repr(stop_word)}")
                if isinstance(stop_word, str):
                    if stop_word in text:
                        print(f"[MATCH] found stop_word '{repr(stop_word)}' in text!")
                        is_finished = True
                        break
                elif isinstance(stop_word, list):
                    cur_tokens = input_ids[i][-len(stop_word):].tolist()
                    print(f"[DEBUG] last tokens: {cur_tokens} vs stop_word: {stop_word}")
                    if cur_tokens == stop_word:
                        print(f"[MATCH] found stop_word tokens {stop_word} at the end!")
                        is_finished = True
                        break
            self.is_done[i] = is_finished
        return self.is_done
