...
RESULTS_JSONL = "/mnt/cephfs1/runqi/nanata/data/tool_rl/thyme_qwen2_5/Thyme/thyme_output_continue_thinklite"
REPLACE_IMAGE_DIR = '/mnt/cephfs1/runqi/nanata/data/tool_rl/thyme_qwen2_5/Thyme/thyme_output_continue_thinklite/temp_images'
os.makedirs(RESULTS_JSONL, exist_ok = True)
os.makedirs(REPLACE_IMAGE_DIR, exist_ok = True)
...
def get_first_image_path(code):
    """
    查找 code 的第一个 xxx_path = 'yyy'，返回变量名和路径，没有则返回 (None, None)
    """
    pattern = r'(\w+_path)\s*=\s*[\'"]([^\'"]+)[\'"]'
...
def fix_image_path_literal(code_string: str) -> str:
    code_string = code_string.replace('\\"', '"') 

    whitespace_pattern = re.compile(r'"(.*?)\s+/"')
    code_string = whitespace_pattern.sub(r'"\1/"', code_string)
...
class GRPOTrainer(RLHFTrainerMixin, SwiftMixin, HFGRPOTrainer):
    ...
    def process_round(self, gen_inputs, gen_outputs, previous_execution_contexts, cur_round=1000):
        ...
        cache_dir = f"/mnt/cephfs1/runqi/nanata/data/tool_rl/thyme_qwen2_5/Thyme/RL_case_cache_new/{self.start_time}_{self.accelerator.process_index}"
        ...
        for idx in range(len(gen_outputs)):
            ...
            if gen_outputs[idx] is not None:
                ...
                original_prefix = './temp_images/'
                replacement_target_path = REPLACE_IMAGE_DIR
                response = response.replace(original_prefix, replacement_target_path)
                response = re.sub(
                        r'([\'\"])\.\/temp_images\/',
                        r'\1' + replacement_target_path,
                        response
                    )
                response = response.replace(replacement_target_path + '/', replacement_target_path)
                
                # print(f"-----------------------模型response: {response}-----------------------")
                ...
                if ( "images" not in gen_inputs[idx] or
                    ...
                    image_path = "/mnt/cephfs1/runqi/nanata/data/mathdata/We-Math2.0-Pro/data/images/3690.png"
                else:
                ...
                if code_match:
                    ...
                    code_to_execute = fix_image_path_literal(code_to_execute)
                    var, img_path = get_first_image_path(code_to_execute)
                    skip_sandbox = False
                    if var is not None and img_path is not None:
                        if not os.path.exists(img_path):
                            skip_sandbox = True

                    if skip_sandbox:
                        print(f"[SANDBOX][CASE {case_id}][idx {idx}] skipped, image `{img_path}` not found.")
                        gen_inputs[idx]["messages"][-1]["content"] += f"Sandbox for {case_id}: skipped due to missing image: {img_path}\n"
                        gen_inputs[idx]["messages"][-1]["content"] += f"Sandbox for {case_id}: No output captured."
                        gen_inputs[idx]["messages"][-1]["content"] += "</sandbox_output>"
                        previous_execution_contexts[idx] = None

                    else:
                        processed_img_paths, captured_stdout, error_msg, current_execution_context = execute_code_in_sandbox(
                            code_to_execute, image_path, temp_output_dir=replacement_target_path, item_id=case_id, previous_execution_context=previous_execution_contexts[idx]
                        )
                        ...
                        if not processed_img_paths:
                            print("="*50)
                            print(f"[SANDBOX][CASE {case_id}][idx {idx}] code:\n{code_to_execute}\n")
                            if error_msg is not None:
                                print(f"[SANDBOX][ERROR] {error_msg}")
                                gen_inputs[idx]["messages"][-1]["content"] += error_msg
                            else:
                                print("[SANDBOX] No image generated, no error reported.")
                                gen_inputs[idx]["messages"][-1]["content"] += f'Sandbox for {case_id}: No output captured.'
                            print("="*50)
    ...
    def _o3_infer(self, inputs: InputsType) -> Tuple[InputsType, OutputsType]:
        ...
        while iteration_limit > 0:
            ...
            try:
                ...
                if self.accelerator.process_index == 1:
                    print(f"OS Error: {e}")
                with open(f"{RESULTS_JSONL}/debug_os_error.jsonl", "a") as fout:
                ...
                if self.accelerator.process_index == 1:
                    print("Out of memory!")
                with open(f"{RESULTS_JSONL}/debug_oom.jsonl", "a") as fout:
                    fout.write(f"Wrong Case: {e}\n")
                    for i in range(len(gen_inputs)):
                        fout.write(json.dumps(gen_inputs[i]) +"\n")
                        fout.write(json.dumps(gen_outputs[i]) +"\n")
                ...
                with open(f"{RESULTS_JSONL}/debug_value.jsonl", "a") as fout:
                    fout.write(f"Wrong Case: {e}\n")
                    for i in range(len(gen_inputs)):
                        fout.write(json.dumps(gen_inputs[i]) +"\n")
                        fout.write(json.dumps(gen_outputs[i]) +"\n")
                ...
                with open(f"{RESULTS_JSONL}/debug_idx.jsonl", "a") as fout:
                    fout.write(f"Wrong Case: {e}\n")
                    for i in range(len(gen_inputs)):
                        fout.write(json.dumps(gen_inputs[i]) +"\n")
                        fout.write(json.dumps(gen_outputs[i]) +"\n")
                ...
                with open(f"{RESULTS_JSONL}/debug_runtime.jsonl", "a") as fout:
                    fout.write(f"Wrong Case: {e}\n")
                    for i in range(len(gen_inputs)):
                        fout.write(json.dumps(gen_inputs[i]) +"\n")
                        fout.write(json.dumps(gen_outputs[i]) +"\n")
                ...
                if self.accelerator.process_index == 1:
                    print(f"Exception: {e}")
                with open(f"{RESULTS_JSONL}/debug_others.jsonl", "a") as fout:
                    fout.write(f"Wrong Case: {e}\n")
                    for i in range(len(gen_inputs)):
                        fout.write(json.dumps(gen_inputs[i]) +"\n")
                        fout.write(json.dumps(gen_outputs[i]) +"\n")
    ...
    def _score_completions(self, inputs: InputsType) -> Tuple[torch.Tensor, torch.Tensor, List[str]]:
        ...
        if (total_rewards_per_func_before - total_rewards_per_func).sum() != 0:
            torch.save(data_to_save, f'{RESULTS_JSONL}/add_mask.pth')
