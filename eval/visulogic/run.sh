...
DATA_PATH="../../data/visulogic.jsonl"
MODEL_PATH="../../model/"
...
if [ ! -f "parallel_inference.py" ]; then
    echo "Error: parallel_inference.py not found!"
    exit 1
fi
...
do
  ...
  CUDA_VISIBLE_DEVICES=$i python parallel_inference.py \
    --gpu_id $i \
...
python extract.py \
    ...
    --output-path "$EXTRACT_OUTPUT_FILE"


python judge.py \
    --model-type "qwen2_5" \
    --json-path "$EXTRACT_OUTPUT_FILE" \
    --model-path "$JUDGE_MODEL"\
    --result-file "$JUDGE_OUTPUT_FILE"
