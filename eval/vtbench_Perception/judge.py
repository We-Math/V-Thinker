...
# -----------------------------
ENGLISH_FINAL_PROMPT_IMAGE_JUDGE = """ Task Description: You are an expert visual evaluator. Your task is to determine if the point(s) marked in the [Generated Image] are conceptually correct according to the Instruction, using the [Ground Truth Image] as a reference for the correct concept.
...
def build_multimodal_prompts(items):
    prompts_data = []
    for it in items:
        instruction = it.get("original_item", {}).get("perception_question", "")
        orig_img_path = it.get("original_item", {}).get("image_path", "")
        gen_img_path = it.get("aux_path", "")
        gt_img_path = it.get("original_item", {}).get("perception_answer_path", "")
        print(gt_img_path)
        if not instruction:
        ...
        try:
            ...
            gt_img_pil = Image.open(it.get("original_item", {}).get("perception_answer_path", "")).convert("RGB")
        except Exception as e:
